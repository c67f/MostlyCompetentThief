 ...
 * @return {Number}
 */
const round = (v, decimalLength) => { const m = 10 ** (decimalLength || 0); return _round(v * m) / m; };
/**
 * @param  {Number} v
...
const render = (tickable, time, muteCallbacks, internalRender, tickMode) => {
    ...
    if (forceTick ||
        autoTick && (time >= updateStartTime && time <= updateEndTime || // Normal render
        ...
        if (!_hasChildren) {
            ...
            while (tween) {
                ...
                if ((seekMode || ((tweenCurrentTime !== tweenChangeDuration || absoluteTime <= tweenAbsEndTime + (tweenNextRep ? tweenNextRep._delay : 0)) &&
...
class Timer extends Clock {
    ...
    constructor(parameters = {}, parent = null, parentPosition = 0) {
        super();
        const { id, delay, duration, reversed, alternate, loop, loopDelay, autoplay, frameRate, playbackRate, onComplete, onLoop, onBegin, onUpdate, } = parameters;
        if (globals.scope)
    ...
    add(a1, a2, a3) {
        ...
        if (isAnim || isTimer || isFunc) {
            ...
            if (isAnim) {
                ...
                addTlChild(
                /** @type TimerParams */ (isTimer ? a1 : { onComplete: a1, duration: minValue }), this, parseTimelinePosition(this, /** @type TimePosition */ (a2)));
            }
...
class ScrollObserver {
    ...
    constructor(parameters) {
        ...
        const syncMethods = isMethods ? /** @type {String} */ (sync).split(' ').map((/** @type {String} */ m) => () => {
...
class Draggable {
    ...
    constructor(target, parameters = {}) {
        ...
        if (isUnd(paramX) || paramX === true) {
            ...
            if (!isUnd(paramXObject.snap))
                this.snapX = paramXObject.snap;
        ...
        if (isUnd(paramY) || paramY === true) {
            ...
            if (!isUnd(paramYObject.snap))
                this.snapY = paramYObject.snap;
    ...
    animateInView(duration = 350, ease = eases.out(5)) {
        ...
        if (this.isOutOfBounds([bt, br, bb, bl], x, y)) {
            ...
            if (!disabledX)
                this.animate[this.xProp](destX, duration, ease);
            if (!disabledY)
                this.animate[this.yProp](destY, duration, ease);
        }
    }
    /**
    ...
    handleDown(e) {
        ...
        if (this.grabbed || /** @type {HTMLInputElement}  */ ($eTarget).type === 'range' || ($eTarget[draggableSymbol] && $eTarget !== this.$trigger))
            return;
        remove(this.scroll);
    ...
    handleUp() {
        ...
        if (this.isOutOfBounds(this.containerBounds, x, y) && cf) {
            ...
            if (!disabledX)
                this.animate[this.xProp](destX, duration, easeX);
            if (!disabledY)
                this.animate[this.yProp](destY, duration, easeY);
        }
        else {
            // The target ends inside the container: simple ease
            const insideDur = duration * .5;
            if (!disabledX)
                this.animate[this.xProp](destX, insideDur, ease);
            if (!disabledY)
                this.animate[this.yProp](destY, insideDur, ease);
        }
        // }
        this.destX = destX;
        this.destY = destY;
...
if (isBrowser) {
    if (!win.AnimeJS)
        win.AnimeJS = [];
    win.AnimeJS.push({ version: '4.0.0-beta.100', engine });
