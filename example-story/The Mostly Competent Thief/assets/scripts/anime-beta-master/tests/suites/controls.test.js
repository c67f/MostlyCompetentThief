...
suite('Controls', () => {
  test('Reverse an animation', resolve => {
  ...
  test('Reverse a reversed animation', resolve => {
    const $target = /** @type {HTMLElement} */(document.querySelector('#target-id'));
    let hasReversed = false;
    animate($target, {
      translateX: 100,
  ...
  test('Reverse a looped animation', resolve => {
    const $target = /** @type {HTMLElement} */(document.querySelector('#target-id'));
    let hasReversed = false;
    animate($target, {
      translateX: 100,
      ...
      onUpdate: self => {
        if (self.currentTime >= (100 * 2.5) && !hasReversed) {
          ...
          setTimeout(() => {
            expect(self.currentTime).to.be.below(timeStamp);
            expect(self._currentIteration).to.equal(1);
            resolve();
          }, 33);
        }
      },
  ...
  test('Reverse a looped alternate animation', resolve => {
    const $target = /** @type {HTMLElement} */(document.querySelector('#target-id'));
    let hasReversed = false;
    animate($target, {
      translateX: 100,
  ...
  test('Reverse a looped alternate reversed animation with odd iteration count', resolve => {
    const $target = /** @type {HTMLElement} */(document.querySelector('#target-id'));
    let hasReversed = false;
    animate($target, {
      translateX: 100,
  ...
  test('Reverse a looped alternate reversed animation with even iteration count', resolve => {
    const $target = /** @type {HTMLElement} */(document.querySelector('#target-id'));
    let hasReversed = false;
    animate($target, {
      translateX: 100,
  ...
  test('Reset a playing animation', () => {
    ...
    expect(animation1._currentIteration).to.equal(0);
    ...
    expect(animation1.completed).to.equal(false);
    expect(animation1._currentIteration).to.equal(1);
    animation1.pause();
  ...
  test('Reset a playing timeline', () => {
    ...
    expect(tl._currentIteration).to.equal(0);
    forEachChildren(tl, child => {
      ...
      expect(child._currentIteration).to.equal(0);
    });
    tl.seek(3000);
    tl.play();
    ...
    expect(tl.completed).to.equal(false);
    expect(tl._currentIteration).to.equal(1);
    tl.pause();
    tl.seek(4000);
