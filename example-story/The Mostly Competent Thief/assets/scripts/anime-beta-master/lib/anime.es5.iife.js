...
var anime = (function (exports) {
          ...
          if (_nodeList) {
            for (var j = 0, jl = _nodeList.length; j < jl; j++) {
              ...
              if (!isNil(subItem)) {
                ...
                for (var k = 0, kl = result.length; k < kl; k++) {
                  if (result[k] === subItem) {
                    isDuplicate = true;
                    break;
                  }
            ...
            for (var _j = 0, _jl = result.length; _j < _jl; _j++) {
              if (result[_j] === item) {
                _isDuplicate = true;
                break;
              }
    ...
    if (!springEasing) {
      ...
      springEasing = {
        ...
        solver: function solver(t) {
          return ceil(_solver(duration * t) * K) / K;
        }
      };
  ...
  var ScrollObserver = function () {
    function ScrollObserver(parameters) {
      var _this2 = this;
    ...
    _createClass(ScrollObserver, [{
      ...
      value: function debug() {
        ...
        var labels = ['enter', 'leave', 'enter', 'leave'];
        this.coords.forEach(function (v, i) {
          ...
          $text.innerHTML = "".concat(labels[i], ": ").concat(_this3.thresholds[i]);
      ...
      value: function handleScroll() {
        ...
        if (isInView) {
          ...
          if (!this.isInView) {
            ...
            this.onEnter(this);
            if ($debug) {
              $debug.style.zIndex = "".concat(this.container.zIndex++);
            }
            if (this.backward) {
  ...
  var Draggable = function () {
    function Draggable(target) {
      ...
      this.enabled = false;
      this.animate.animations[this.disabled[0] ? yProp : xProp].onRender = function () {
        var hasMoved = _this3.grabbed && _this3.updated;
        var hasReleased = !hasMoved && _this3.released;
        if (hasMoved || hasReleased) {
          var x = _this3.x;
          var y = _this3.y;
          _this3.deltaX = _this3.coords[2] - x;
          _this3.deltaY = _this3.coords[3] - y;
          _this3.coords[2] = x;
          _this3.coords[3] = y;
          _this3.progresses[0] = mapRange(x, _this3.containerBounds[3], _this3.containerBounds[1], 0, 1);
          _this3.progresses[1] = mapRange(y, _this3.containerBounds[0], _this3.containerBounds[2], 0, 1);
        }
        if (hasMoved) {
      ...
      this.animate.animations[this.disabled[0] ? yProp : xProp].onComplete = function () {
    ...
    _createClass(Draggable, [{
      ...
      key: "scrollInView",
      value: function scrollInView(gap) {
        var _this4 = this;
      ...
      key: "animateInView",
      value: function animateInView() {
        var duration = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 350;
        var ease = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : eases.out(5);
        this.updateBoundingValues();
        var x = this.x;
        var y = this.y;
        ...
        var bt = this.scroll.y - this.targetBounds[0] + cpt;
        var br = this.scroll.x - this.targetBounds[1] - cpr;
        var bb = this.scroll.y - this.targetBounds[2] - cpb;
        var bl = this.scroll.x - this.targetBounds[3] + cpl;
      ...
      value: function handleUp() {
        ...
        if (this.isOutOfBounds(this.containerBounds, x, y) && cf) {
          var bounceX = !disabledX && (x > br || x < bl);
          var bounceY = !disabledY && (y > bb || y < bt);
