...
import { animate, utils, eases, spring } from '../../src/anime.js';
...
suite('Eases', () => {
  ...
  const fnTypes = ['in', 'out', 'inOut', 'outIn'];
  ...
  test('Custom power ease: in(x), out(x), inOut(x), outIn(x)', () => {
  ...
  test('Custom elastic ease: inElastic(x, y), outElastic(x, y), inOutElastic(x, y), outInElastic(x, y)', () => {
    const anim1 = animate('#target-id', createEasingParam('in(1)'));
    anim1.seek(50);
    expect(getOpacityValue()).to.equal(.50);
    utils.set('#target-id', {opacity: 0});
  ...
  test('Spring ease parameters affect animation\'s duration', () => {
    expect(animate('#target-id', createEasingParam(spring())).duration).to.equal(2000);
    expect(animate('#target-id', createEasingParam(spring(10, 100, 10, 0))).duration).to.equal(18333);
    expect(animate('#target-id', createEasingParam(spring(1, 50, 10, 0))).duration).to.equal(1833);
    expect(animate('#target-id', createEasingParam(spring(1, 100, 50, 0))).duration).to.equal(1333);
    expect(animate('#target-id', createEasingParam(spring(1, 100, 10, 10))).duration).to.equal(1833);
  });
  ...
  test('Cubic bézier outIn: "cubicBezier(0,1,1,0)" / eases.cubicBezier(0,1,1,0)', () => {
    const cubicBezierOutIn = animate('#target-id', createEasingParam(eases.cubicBezier(0,1,1,0)));
    cubicBezierOutIn.seek(50);
    expect(getOpacityValue()).to.be.equal(.5);
    const cubicBezierOutInString = animate('#target-id', createEasingParam('cubicBezier(0,1,1,0)'));
    cubicBezierOutInString.seek(50);
    expect(getOpacityValue()).to.be.equal(.5);
  });
