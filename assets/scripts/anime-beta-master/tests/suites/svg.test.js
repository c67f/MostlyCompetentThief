...
suite('SVG', () => {
  test('svg.createDrawable', resolve => {
    ...
    animate(svg.createDrawable(['line', 'circle', 'polygon', 'polyline', 'rect', 'path']), {
      ...
      duration: 10,
      onComplete: () => {
        expect(line1El.getAttribute('stroke-dasharray')).to.equal('100000 1');
        expect(line2El.getAttribute('stroke-dasharray')).to.equal('100000 1');
        expect(circleEl.getAttribute('stroke-dasharray')).to.equal('100000 1');
        expect(polygonEl.getAttribute('stroke-dasharray')).to.equal('100000 1');
        expect(polylineEl.getAttribute('stroke-dasharray')).to.equal('100000 1');
        expect(rectEl.getAttribute('stroke-dasharray')).to.equal('100000 1');
        resolve();
      }
    });

    // NOTE: Initial stroke-dashoffset values are not equals 0 to prevent the edges bleeding

    // expect(line1El.getAttribute('stroke-dashoffsetbug')).to.equal('0');
    // expect(line2El.getAttribute('stroke-dashoffset')).to.equal('0');
    // expect(circleEl.getAttribute('stroke-dashoffset')).to.equal('0');
    // expect(polygonEl.getAttribute('stroke-dashoffset')).to.equal('0');
    // expect(polylineEl.getAttribute('stroke-dashoffset')).to.equal('0');
    // expect(rectEl.getAttribute('stroke-dashoffset')).to.equal('0');

    expect(+line1El.getAttribute('stroke-dasharray').split(' ')[1]).to.be.above(100000);
    expect(+line2El.getAttribute('stroke-dasharray').split(' ')[1]).to.be.above(100000);
    expect(+circleEl.getAttribute('stroke-dasharray').split(' ')[1]).to.be.above(100000);
    expect(+polygonEl.getAttribute('stroke-dasharray').split(' ')[1]).to.be.above(100000);
    expect(+polylineEl.getAttribute('stroke-dasharray').split(' ')[1]).to.be.above(100000);
    expect(+rectEl.getAttribute('stroke-dasharray').split(' ')[1]).to.be.above(100000);
  });
  ...
  test('Animating non existant attributes', resolve => {
    ...
    expect(+path2El.getAttribute('stroke-dasharray').split(' ')[1]).to.equal(1);
